...
def render_comparison_report(metrics: list[RunMetrics], plot_paths: list[Path], flowchart_path: Path | None) -> str:
    lines = [
        ...
        "## 테스트 판정 기준",
        "",
        "- PASS는 코드가 생성 SQL의 validation 통과와 preview 준비 상태를 확인한 쿼리 실행 가능성 지표입니다. 단, deterministic advisory check가 명백한 요청-SQL-sample 불일치를 발견하면 query result는 CHECK로 낮춥니다.",

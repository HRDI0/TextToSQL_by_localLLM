...
이 문서는 자연어 SQL workflow에 유사 검색 기반 추천 기능을 추가하는 공개용 설계 문서다. 핵심 목표는 없는 컬럼명이나 값이 입력되었을 때 시스템이 조건을 몰래 치환하지 않고, preview 화면에서 사용자가 선택할 수 있는 추천 조건을 제시하는 것이다.
...
- 추천 조건도 항상 다시 preview부터 실행한다.
- 추천 조건으로 생성된 SQL도 기존 validation, preview, approval, fingerprint gate를 그대로 통과해야 한다.
- UPDATE 요청은 잘못된 fuzzy match가 row를 잘못 수정할 수 있으므로 SELECT보다 더 보수적으로 다룬다.

## 2. 전체 Workflow 삽입 위치

기존 workflow는 natural-language request를 IR, SQL candidate, validation, preview, approval, execution으로 순서대로 처리한다. 추천 기능은 `parse_ir_request` 뒤에 side-effect 없는 추천 노드로 추가한다.
...
추천 노드는 `selection_request`나 `modification_logic`을 직접 수정하지 않는다. 추천 결과는 별도 state에 저장하고 preview/output 단계에서 사용자에게 보여준다.
...
sequenceDiagram
    ...
    Graph-->>UI: SQL preview, sample rows, 추천 조건 반환
    UI-->>User: "원하는 결과가 아닌가요?" 영역 표시
```

이 흐름에서 추천은 advisory metadata다. 추천 결과가 있어도 기존 SQL 생성/검증/preview path를 우회하지 않는다.

## 5. Recommendation-only Preview Loop

사용자가 추천 조건을 선택하면 기존 preview 결과를 바로 승인하지 않고, 추천 문구를 새 입력으로 사용해 다시 preview를 실행한다.

```mermaid
flowchart TD
    Preview["현재 Preview 표시"]
    Ask["원하는 결과가 아닌가요?"]
    Items["추천 조건 목록\n컬럼 후보 / 값 후보 / 대안 조건"]
    Choose{"사용자가 추천 조건을 선택했는가?"}
    Rerun["추천 조건으로 다시 Run Preview\napproved=false"]
    Validate["기존 validation / preview / fingerprint gate"]
    Approve{"사용자가 preview를 승인했는가?"}
...
이 구조에서는 추천 조건 선택과 승인 실행이 분리된다. 추천 조건을 눌러도 write가 발생하지 않으며, 새 preview를 다시 확인해야 한다.
...
최종 output 또는 preview JSON에는 다음 사용자 문구를 포함한다.

```text
원하는 결과가 아닌가요?
아래 추천 조건으로 다시 Preview 해볼 수 있습니다.
```
...
| --- | --- | --- | --- |
| high | 명확한 alias 또는 높은 fuzzy score | 다시 Preview 버튼 표시 | 자동 실행 없음 |
| medium | 후보가 여러 개이거나 score가 애매함 | 후보 설명과 확인 필요 표시 | 자동 실행 없음 |
| low | 후보 부족 또는 위험한 match | 추천 대신 unresolved 안내 | SQL 생성 우회 없음 |

UPDATE의 WHERE 조건에 들어가는 후보는 high confidence라도 preview에서 affected row count와 sample row를 반드시 확인해야 한다.
...
추천은 사용자의 다음 preview 입력을 돕는 UX layer일 뿐이다. SQL 생성과 실행 권한은 기존 deterministic validation과 approval gate가 가진다.
...
| Output | `app/langgraph_workflow/stage_04_output.py` | preview/output JSON에 추천 포함 |
| UI | `app/streamlit_langgraph_test.py` | "원하는 결과가 아닌가요?" 추천 영역과 rerun preview 버튼 |

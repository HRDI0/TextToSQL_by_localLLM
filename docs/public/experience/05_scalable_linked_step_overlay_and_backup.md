...
- aggregate overlay SQL은 필요한 column만 projection하도록 줄였습니다.
- COUNT 요청은 row id 합산이 아니라 `COUNT(*)`로 처리했습니다.
...
- overlay SQL은 row 수에 비례하지 않고, aggregate path에서는 필요한 column 중심으로 projection됩니다.
...
첫째, detail SELECT overlay는 여전히 표시 목적상 많은 column을 펼칠 수 있습니다. 공개 화면이나 report에서는 필요한 column만 보여주는 presentation 최적화가 남아 있습니다.

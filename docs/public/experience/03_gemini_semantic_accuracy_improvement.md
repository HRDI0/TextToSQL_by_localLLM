...
최종 결과는 test1과 test2를 합산해 OMO 직접 semantic review 기준 94/100이었습니다. 모든 실행은 preview-only로 진행했고, 승인값을 전달하지 않았기 때문에 raw DB write는 실행되지 않았습니다.
...
사용자는 test1과 test2를 다시 진행하면서 OMO가 직접 Semantic PASS를 분석하고, 정확도를 90%까지 올리라고 지시했습니다. 동시에 다음 제약 조건이 있었습니다.
...
Query PASS는 SQL 후보가 생성되고 preview 가능한지를 보는 지표입니다. 반면 Semantic PASS는 자연어 요청, SQL, sample row evidence가 실제 의도와 맞는지를 OMO가 직접 판단한 지표입니다.
...
6. OMO가 raw 결과를 다시 읽고 top-level Semantic과 Step Semantic을 직접 판정했습니다.
...
| test1 OMO Semantic PASS | 49/50 |
| test2 OMO Semantic PASS | 45/50 |
| 합산 OMO Semantic PASS | 94/100 |

90% 목표는 달성했습니다. 다만 semantic 실패가 0개가 된 것은 아닙니다. 남은 실패는 주로 다음 유형이었습니다.

- 요청한 table routing이 일부 step에서 어긋남
- 요청한 group-by 기준이 빠짐
- 사용자가 실제 numeric update를 요구한 문장이 derived-value insert로 해석됨

이 실패는 이번 수정 범위를 넘어서는 parser/table-routing/aggregate guard 문제로 분류했습니다. 목표인 90%를 넘겼고, 추가 수정은 과적합 위험이 커질 수 있어 여기서 멈췄습니다.
...
- parser 단계에서 매체/table routing을 step별로 더 강하게 검증합니다.
- aggregate 요청에서 group-by 누락을 validation error로 올립니다.
- 실제 numeric update 요청과 derived-value label 요청을 더 정교하게 구분합니다.
- derived-value 저장 구조를 후속 SELECT/AGGREGATE가 직접 join해서 사용할 수 있는 step executor를 추가합니다.
- linked step plan의 `depends_on`을 runtime preview/approval/invalidation과 완전히 연결합니다.

이번 작업은 이 중 “파생 label을 raw update로 오해하지 않기”와 “파생 조건 때문에 live 조건까지 잃지 않기”를 먼저 해결한 것입니다.

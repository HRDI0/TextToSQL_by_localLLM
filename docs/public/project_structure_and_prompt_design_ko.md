...
이 문서는 자연어 기반 SQL workflow를 설명하는 공개용 구조도와 프롬프트 설계서다. 목적은 프로젝트의 주요 디렉터리, LangGraph 실행 흐름, LLM prompt 역할, SQL 안전장치, 적용 전 확인과 사용자 확정 정책을 한눈에 이해할 수 있게 정리하는 것이다.
...
이 프로젝트는 CSV/XLSX 형태의 tabular source data를 MariaDB staging table에 적재한 뒤, 사용자의 자연어 조회/수정 요청을 review-first LangGraph workflow로 처리한다. 핵심 원칙은 LLM이 실행 SQL을 직접 결정하지 않고, deterministic Python code와 live DB metadata가 SQL 후보를 검증한 뒤 사람이 샘플 영향 범위를 확인하고 확정해야만 실행한다는 점이다.
...
flowchart TD
    ...
    subgraph Workflow["app/langgraph_workflow"]
        ...
        Output["stage_04_output.py\nimpact view + confirmation + final JSON"]
    end
...
| Workflow | 자연어 요청을 IR, SQL 후보, 적용 전 확인 결과, 실행 결과로 변환 | `app/langgraph_workflow/` |
| UI | 사람이 샘플 데이터를 보고 확정/제외하는 local validation surface | `app/streamlit_langgraph_test.py` |
| Test Runner | markdown case를 review-only로 실행해 회귀 확인 | `scripts/run_langgraph_test_cases.py` |
...
flowchart TD
    ...
    Start --> LoadMeta --> ParseIR --> Recommend --> GenSelect --> ValSelect --> FetchRows --> RuleLookup --> GenCrud --> ValSql --> BuildPreview --> Confirm --> Execute --> Final --> End
```
...
| Recommendation | `build_resolution_recommendations` | IR, live schema, catalogs | `query_recommendations`, `resolution_candidates` | 모호한 term의 추천 후보만 만들고 원본 IR/SQL은 바꾸지 않는다. |
...
flowchart TD
    ...
    Deterministic["Deterministic renderer\nrow-level SELECT / aggregate / derived review"]
    ...
    IntentRoute -- "SELECT_DETAIL" --> Deterministic
...
```json
{
  "intent_type": "SELECT_DETAIL | UPDATE_NUMERIC_VALUE | ADD_DERIVED_COLUMN | SELECT_AGGREGATE | ASK_CLARIFICATION",
  "selection": {
    "customer": "default",
    "period": {},
    "source_channels": [],
...
| --- | --- | --- | --- |
| 저장 규칙 match | 저장 rule action을 deterministic compiler로 변환 | 사용 안 함 | validation과 사용자 확정 통과 시 가능 |
| `SELECT_DETAIL` | 첫 조회 범위와 추가 조건을 결합해 row-level SELECT 생성 | 사용 안 함 | SELECT 결과 조회용 |
| `SELECT_AGGREGATE` | metric/dimension dictionary와 IR로 aggregate SELECT 생성 | 사용 안 함 | SELECT 결과 조회용 |
| `ADD_DERIVED_COLUMN` | raw table ALTER가 아니라 derived-value INSERT 후보 생성 | 사용 안 함 | review-only, `execution_allowed=False` |
| UPDATE 계열 | LLM action-plan JSON + deterministic SQL compiler | action 선택에만 사용 | validation과 사용자 확정 통과 시 가능 |
| `ASK_CLARIFICATION` | SQL 생성 차단 | 사용 안 함 | 실행 안 함 |

`ADD_DERIVED_COLUMN`은 raw staging table에 새 컬럼을 추가하지 않는다. 대신 source row identity와 derived key/value를 `rule_engine_derived_value`에 기록하는 후보 SQL을 만들고, 현재 workflow에서는 review-only로 둔다.

## 11. SQL Validation Gate

`validate_generated_sql`은 LLM validation이 아니라 script-only validation이다. SQL text, metadata, live schema, protected policy가 서로 일치해야 적용 전 확인으로 넘어갈 수 있다.
...
| Predicate metadata check | 적용 전 확인 결과와 SQL WHERE가 동일한 predicate column metadata를 가져야 한다. |
| Fingerprint gate | SQL fingerprint와 review fingerprint가 확정 시점에 동일해야 한다. |

## 12. 적용 전 확인과 확정 정책

`stage_04_output.py`는 SQL 후보를 즉시 실행하지 않고 적용 전 확인 JSON을 만든다. 확인 결과에는 rendered SQL, SQL fingerprint, review fingerprint, affected row count, sample row 변화 예시, validation result가 포함된다.
...
sequenceDiagram
    ...
    Reviewer->>UI: 적용 전 결과 확인
    UI->>Graph: Invoke workflow with approved=false
    Graph->>DB: Read live schema and sample rows
    Graph-->>UI: IR JSON, rendered SQL, sample rows, fingerprints
    UI-->>Reviewer: 샘플 데이터 표시
    Reviewer->>UI: 승인 또는 적용하지 않음
    alt Confirm with matching fingerprints
        UI->>Graph: Existing review result + confirmed fingerprints
        Graph->>DB: Execute guarded SQL
        Graph-->>UI: execution_result
    else Skip or mismatch
...
3. 사용자가 명시적으로 승인함
4. 승인된 SQL fingerprint와 review fingerprint가 현재 확인 결과의 fingerprint와 일치함

조건이 하나라도 실패하면 실행 결과는 skipped 상태가 된다. review-only SQL 후보도 실행하지 않는다.
...
적용 전 확인 결과는 execution output이 아니다. validation failure, user rejection, fingerprint mismatch, no connection, review-only candidate는 모두 skipped result로 남는다.
...
LLM 요청은 IR 구조화와 UPDATE action-plan 생성에만 사용한다. SQL 검증, 적용 전 확인 생성, confirmation gate는 LLM 판단에 의존하지 않는다.
...
유사 검색 추천 구조는 별도 문서 [fuzzy_search_recommendation_architecture.md](./fuzzy_search_recommendation_architecture.md)에 정리되어 있다. 이 확장은 원본 query를 자동 치환하지 않고, 적용 전 확인 화면에서 “원하는 결과가 아닌가요?” 형태의 추천 조건을 제시하는 UX layer로 설계한다.
...
flowchart LR
    ...
    Rerun["다시 확인\napproved=false"]
    Gate["Same validation + confirmation gates"]

    Strict --> Rec
    Rec --> User --> Rerun --> Gate
```

추천 조건은 실행 권한을 갖지 않는다. 사용자가 선택한 뒤에도 반드시 다시 적용 전 확인을 실행하고, 기존 validation/confirmation/fingerprint gate를 통과해야 한다.
...
이 workflow의 핵심은 LLM을 SQL 실행 주체로 두지 않는 것이다. LLM은 자연어를 IR로 구조화하거나 UPDATE action 후보를 JSON으로 제안한다. SQL 후보 생성, schema 검증, parameter 검증, 적용 전 확인 생성, 승인 fingerprint 확인, 실행 여부 결정은 deterministic Python code와 DB metadata가 담당한다.
